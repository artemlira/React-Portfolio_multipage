// import GerichtImg from '../assets/images/Home_Projects/Gericht _1.png';
// import GerichtWebp from '../assets/images/Home_Projects/gericht-1.webp';

export const projects = [
  {
    id: 1,
    // img: GerichtImg,
    img: '../assets/images/Home_Projects/Gericht _1.png',
    // imgWebp: GerichtWebp,
    imgWebp: '../assets/images/Home_Projects/gericht-1.webp',
    skills: ['React', 'JavaScript', 'SCSS', 'HTML', 'CSS'],
    title: 'Gerich',
    shortDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів.',
    fullDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів. Застосунок написано з використанням фреймворка Реакт, у якому застосовувалося: useEffect, useState, useContext, Router Dom v.6, Swiper, Framer-motion, Styled-components тощо.',
    git: 'https://github.com/artemlira/React-Gericht_multipage.git',
    deploy: 'https://react-gericht-multipage.netlify.app/',
  },
  {
    id: 2,
    // img: GerichtImg,
    img: '../assets/images/Home_Projects/Gericht _1.png',
    // imgWebp: GerichtWebp,
    imgWebp: '../assets/images/Home_Projects/gericht-1.webp',
    skills: ['React', 'JavaScript', 'SCSS', 'HTML', 'CSS'],
    title: 'Gerich',
    shortDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів.',
    fullDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів. Застосунок написано з використанням фреймворка Реакт, у якому застосовувалося: useEffect, useState, useContext, Router Dom v.6, Swiper, Framer-motion, Styled-components тощо.',
    git: 'https://github.com/artemlira/React-Gericht_multipage.git',
    deploy: 'https://react-gericht-multipage.netlify.app/',
  },
  {
    id: 3,
    // img: GerichtImg,
    img: '../assets/images/Home_Projects/Gericht _1.png',
    // imgWebp: GerichtWebp,
    imgWebp: '../assets/images/Home_Projects/gericht-1.webp',
    skills: ['React', 'JavaScript', 'SCSS', 'HTML', 'CSS'],
    title: 'Gerich',
    shortDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів.Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів.',
    fullDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів. Застосунок написано з використанням фреймворка Реакт, у якому застосовувалося: useEffect, useState, useContext, Router Dom v.6, Swiper, Framer-motion, Styled-components тощо.',
    git: 'https://github.com/artemlira/React-Gericht_multipage.git',
    deploy: 'https://react-gericht-multipage.netlify.app/',
  },
  {
    id: 4,
    // img: GerichtImg,
    img: '../assets/images/Home_Projects/Gericht _1.png',
    // imgWebp: GerichtWebp,
    imgWebp: '../assets/images/Home_Projects/gericht-1.webp',
    skills: ['React', 'JavaScript', 'SCSS', 'HTML', 'CSS'],
    title: 'Gerich',
    shortDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів.',
    fullDescription: 'Багатосторінковий адаптивний сайт ресторану з безліччю красивої анімації та слайдерів. Застосунок написано з використанням фреймворка Реакт, у якому застосовувалося: useEffect, useState, useContext, Router Dom v.6, Swiper, Framer-motion, Styled-components тощо.',
    git: 'https://github.com/artemlira/React-Gericht_multipage.git',
    deploy: 'https://react-gericht-multipage.netlify.app/',
  },
];

export const skills = [];
